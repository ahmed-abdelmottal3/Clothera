...
export function useProductFilter(products: Product[]) {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      const price = typeof product.priceAfterDiscount === 'number' 
  ...
  const handleFilterChange = (key: string, value: string | number[]) => {
    switch (key) {
      case 'category': setSelectedCategory(value as string); break;
      case 'brand': setSelectedBrand(value as string); break;
      case 'priceRange': setPriceRange(value as number[]); break;
    }
  };
